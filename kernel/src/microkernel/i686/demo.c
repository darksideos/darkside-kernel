...
void pit_ch2_delay(int ticks, void *cb);
...
static unsigned char teletext[][9] = {
	...
	{4, 4, 4, 4, 4, 4, 4, 0, 0},			// |
	{12, 2, 2, 1, 2, 2, 12},				// }
	{0, 0, 8, 21, 2, 0, 0, 0, 0},		// ~, needs to be improved
...
static void put_string(framebuffer_t *fb, const char *str, int x, int y, uint32_t color)
{
	while (*str)
	{
...
static void put_row(framebuffer_t *fb, const char *str, int x, int y, int width, bool inverted)
{
...
void demo(framebuffer_t *fb, int (*ps2kbd_init)(keyboard_ops_t *ops), int (*ps2mouse_init)(mouse_ops_t *ops))
{
	...
	const char* options[] = {"Hello, Darkside.", "Animated graphics demo", "Mouse demo", "Idk what goes here", "GEOOOOOORGE"};
	int sel_index = 0;
