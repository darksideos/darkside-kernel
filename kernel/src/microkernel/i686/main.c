...
void microkernel_init(loader_block_t *_loader_block, bool bsp)
{
	...
	if (bsp)
	{
		...
		{
			printf("Allocating 1 page: 0x%08X\n", pmm_alloc_page(PAGE_DMA, NUMA_DOMAIN_BEST, 0));
		}

		for (uint32_t i = 1; i < 17; i++)
		{
			printf("Allocating %d pages: 0x%08X\n", i, pmm_alloc_pages(i, PAGE_DMA, NUMA_DOMAIN_BEST, 0));
		}
