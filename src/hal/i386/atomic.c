...
{
	__asm__ volatile("lock addl %1, %0" : "+m" (v->counter) : "ir" (incr));
}

/* Atomically subtract 'decr' from 'v->counter' */
void atomic_sub(atomic_t *v, uint32_t decr)
{
	__asm__ volatile("lock subl %1, %0" : "+m" (v->counter) : "ir" (decr));
}
...
{
	uint32_t ret;
	__asm__ volatile("lock xadd %0, %1" : "=r"(ret), "=m"(v->counter): "0"(incr), "m"(v->counter) : "memory");
	return ret;
}
...
{
	uint32_t ret;
	__asm__ volatile("lock xadd %0, %1" : "=r"(ret), "=m"(v->counter): "0"(-decr), "m"(v->counter) : "memory");
	return ret;
}
...
{
	uint32_t ret = newval;
	__asm__ volatile("lock xchgl %0, %1" : "=q" (ret), "=m" (v->counter) : "0" (ret));
	return ret;
}
...
{
	uint32_t ret;
	__asm__ volatile("lock cmpxchgl %2,%1" : "=a" (ret), "+m" (v->counter) : "r" (newval), "0" (oldval) : "memory");
	return ret;
}
