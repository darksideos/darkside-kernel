...
/* NOAH - the init page table is at physical address (remember, paging isn't enabled yet) 0x4000, and the kernel data table at 0x3c00 */
/* NOAH - the init page table is the equivalent of an x86 page directory */
/* NOAH - the kernel data table is a page table used to map kernel data */
...
__attribute__((naked)) void initsys(void)
{
	...
	for(x = 0; x < 4096; x++)
	{
		/* NOAH - this just maps a bunch of physical memory, we should get rid of this -- we don't need it.  FYI it is still necessary to map the peripherals */
	 ...
	 * table is unmapped.
	 */
	/* NOAH - map kernel data to 0xc0000000 */
	 ...
	 */
	/* NOAH - here, we map only as much of kernel data as we need */
	for(x = 0; x < 256; x++)
	{
	 ...
	 */
	/* NOAH - zero the .bss section (why do we need this?) */
	bss = &_physbssstart;
	...
	/* NOAH - note - we could use translation table 1 for kernel stuff and 0 for userspace stuff, but that won't be as easy to implement */
	/* Still, it would be good to rewrite the HAL interface to support different page sizes */
	/* Translation table 1 */
