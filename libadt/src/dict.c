...
static void dict_set(dict_t *dict, const char *key, void *item)
{
	...
	int i;
	for (i = hash_key % dict->total_size; entries[i].key != NULL; i = (i+1) % dict->total_size)
	{
		if (strcmp((char*)entries[i].key, (char*)key))
		{
...
void *dict_get(dict_t *dict, const char *key)
{
	...
	for (int i = hash(key) % dict->total_size; entries[i].key != NULL; i = (i+1) % dict->total_size)
	{
		if (strcmp((char*)entries[i].key, (char*)key)) return entries[i].value;
	}
