...
void map_page(uint32_t tbl, uint32_t virtual_address, uint32_t physical_address, bool present, bool rw, bool user, bool global);

/* Unmap a virtual address */
void unmap_page(uint32_t tbl, uint32_t virtual_address);

/* Create a new blank page directory */
uint32_t create_address_space();

/* Switch the current page directory to a new one */
void switch_address_space(uint32_t tbl);
